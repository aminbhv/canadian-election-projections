...
class RegionNode:
    ...
    def simulate(
        ...
        Args:
            polling_data (dict): Polling data for the country or a specific province
            graph (nx.DiGraph): Voter transition graph
            margin (float): Random margin to apply to polling. Defaults to 0.03.
        ...
        if self.node_type == "province":
            for child in self.children:
                child_poll = polling_data.get(self.name, {})
                child_result = child.simulate(child_poll, graph, margin)
                for party, count in child_result.items():
                    self.results[party] = self.results.get(party, 0) + count
...
def simulate_single_seat(
    ...
    sampled_poll = {
        party: max(0, min(1, adjusted_polling[party] + random.uniform(-margin, margin)))
        for party in adjusted_polling
    }
    total = sum(sampled_poll.values())
    for party in sampled_poll:
        sampled_poll[party] /= total
...
def adjust_polling_graph(
    original_polling: Dict[str, float],
    ...
    Args:
        ...
        influence_factor (float): Factor to weight the graph influence. Defaults to 0.2.

    Returns:
        Dict[str, float]: Adjusted polling data
    """
    if graph is None:
        return original_polling

    adjusted_polling: Dict[str, float] = original_polling.copy()
    for party in original_polling:
        if party == "OTH" or party not in graph.nodes:
            continue
        incoming_influence = 0.0
        for neighbor in graph.predecessors(party):
            if neighbor in original_polling and graph[neighbor][party]["weight"] > 0:
                incoming_influence += original_polling[neighbor] * graph[neighbor][party]["weight"]
        adjusted_polling[party] += influence_factor * incoming_influence

    total = sum(adjusted_polling.values())
    for party in adjusted_polling:
        adjusted_polling[party] /= total

    return adjusted_polling


def _simulate_trial(
    election_tree: RegionNode,
    polling_data: Dict[str, Any],
    voter_graph: Optional[nx.DiGraph],
    margin: float = 0.03,
) -> Dict[str, int]:
    """
    ...
    Args:
        ...
        margin (float): Margin parameter. Defaults to 0.03.
...
def run_simulation(
    polling_data: Dict[str, Any],
    trials: int = 1000,
    voter_graph: Optional[nx.DiGraph] = None,
    ...
    Args:
        ...
        voter_graph (Optional[nx.DiGraph]): Voter transition graph. Defaults to None.

    Returns:
        Tuple containing seat distribution and win probability statistics.
    """
    election_tree = build_election_tree(SEATS_BY_PROVINCE)
    all_parties = {party for province_poll in polling_data.values() for party in province_poll}

    seat_distribution: Dict[str, List[int]] = {party: [] for party in all_parties}
    win_stats: Dict[str, Dict[str, float]] = {
        party: {"majority": 0, "minority": 0, "no_win": 0} for party in all_parties
    }
    ...
    for _ in range(trials):
        ...
        for party, count in results.items():
            seat_distribution[party].append(count)
        update_win_stats(results)

    for party in all_parties:
        maj = win_stats[party]["majority"]
        minr = win_stats[party]["minority"]
        win_stats[party]["no_win"] = trials - maj - minr
        for stat in win_stats[party]:
            win_stats[party][stat] /= trials
...
if __name__ == "__main__":
    ...
    if sample_polling:
        ...
        for party, prob in stats.items():
            print(f"{party}: {100 * prob['majority']:.1f}% majority, {100 * prob['minority']:.1f}% minority")            name (str): Name of the region
            node_type (str): Type of region ('country', 'province', or 'seat')
